pipeline {
  agent any
  environment {
    GIT_REPO = 'git@github.com:nibroos/e-proc-api.git'
    SSH_CREDENTIALS_ID = 'vps-ssh-credentials-e-proc-api-27'
    VPS_USER = credentials('vps-user-27')
    VPS_HOST = credentials('vps-host-27')
    VPS_DEPLOY_DIR = credentials('vps-deploy-dir-e-proc-api-be-27')

    POSTGRES_USER = credentials('vps-postgres-username-e-proc-api-be-27')
    POSTGRES_PASSWORD = credentials('vps-postgres-password-e-proc-api-be-27')
    POSTGRES_DB = credentials('vps-postgres-e-proc-api-be-27')
    POSTGRES_PORT = credentials('vps-postgres-port-e-proc-api-be-27')
    POSTGRES_HOST = credentials('vps-postgres-host-e-proc-api-be-27')

    POSTGRES_USER_TEST = credentials('vps-postgres-username-test-e-proc-api-be-27')
    POSTGRES_PASSWORD_TEST = credentials('vps-postgres-password-test-e-proc-api-be-27')
    POSTGRES_DB_TEST = credentials('vps-postgres-test-e-proc-api-be-27')
    POSTGRES_PORT_TEST = credentials('vps-postgres-port-test-e-proc-api-be-27')
    POSTGRES_HOST_TEST = credentials('vps-postgres-host-test-e-proc-api-be-27')

    GATEWAY_PORT = credentials('vps-gateway-e-proc-api-be-27')
    SERVICE_GRPC_PORT = credentials('vps-service-grpc-e-proc-api-be-27')
    SERVICE_REST_PORT = credentials('vps-service-rest-e-proc-api-be-27')

    REDIS_HOST = credentials('vps-redis-host-e-proc-api-be-27')
    REDIS_PORT = credentials('vps-redis-port-e-proc-api-be-27')
    REDIS_PASSWORD = credentials('vps-redis-password-e-proc-api-be-27')
    REDIS_DB = credentials('vps-redis-db-e-proc-api-be-27')

    REDIS_HOST_TEST = credentials('vps-redis-host-test-e-proc-api-be-27')
    REDIS_PORT_TEST = credentials('vps-redis-port-test-e-proc-api-be-27')
    REDIS_PASSWORD_TEST = credentials('vps-redis-password-test-e-proc-api-be-27')
    REDIS_DB_TEST = credentials('vps-redis-db-test-e-proc-api-be-27')

    JWT_SECRET = credentials('vps-jwt-secret-e-proc-api-27')
    APP_ENV = "test"
